document.addEventListener('DOMContentLoaded', () => {
  ...
  const calculate = () => {
    ...
    if (operatingIncome === null || isNaN(operatingIncome) || sales === null || isNaN(sales)) {
      updateDisplay('operatingMargin', '-');
    } else {
      updateDisplay('operatingMargin', formatPercent(operatingMargin));
    }
    updateDisplay('roe', formatPercent(roe));
    updateDisplay('equityRatio', formatPercent(equityRatio));
    updateDisplay('currentRatio', formatPercent(currentRatio));
...
2. 期間（PL項目について）: 売上高、営業利益、当期純利益などの損益計算書（PL）項目について、「第〇四半期」などの期間が区切られた数字は絶対に取得しないでください。必ず「通期（1年分）」の数字（実績値、または「通期の部」に記載されている会社予想値・業績予想）を最優先で抽出してください。
  ...
  pdfUpload.addEventListener('change', async (e) => {
    ...
    try {
      ...
      const fillField = (id, value) => {
        const el = document.getElementById(id);
        if (!el) return;
        if (value === null && (id === 'inventory' || id === 'operatingIncome')) {
          el.value = "-";
