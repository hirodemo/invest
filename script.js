document.addEventListener('DOMContentLoaded', () => {
  ...
  const formatTimes = (times) => {
    if (times === null || times === undefined || isNaN(times)) return "-";
    return times.toFixed(2) + " 倍";
  };
  ...
  const updateDisplay = (id, value, fallback = "-") => {
    ...
    // Check for explicit "error" or "no value" indicators
    if (value === null || value === undefined || value === "-" || value === "--" || (typeof value === 'number' && isNaN(value))) {
  ...
  const calculate = () => {
    ...
    const inventory = getVal('inventory') || 0; // 棚卸資産（ない場合は0扱い）
    ...
    // => (株価 × 株式数(万株) × 10,000) / 100,000,000 = 株価 × 株式数(万株) / 10000 = 億円
    const marketCap = (stockPrice && sharesOutstanding) ? (stockPrice * sharesOutstanding) / 10000 : null;
    ...
    if (currentAssets !== null && totalLiabilities !== null && marketCap !== null) {
      ...
      // ネットキャッシュ比率 = （流動資産＋投資有価証券×0.7 - 棚卸資産×0.3 - 負債）/時価総額
      // ※investmentSecuritiesにはIFRSの「その他の金融資産(非流動)」が含まれる
      const netCashValue = currentAssets + (investmentSecurities * 0.7) - (inventory * 0.3) - totalLiabilities;
    ...
    if (operatingIncome === null || isNaN(operatingIncome) || sales === null || isNaN(sales)) {
      updateDisplay('res-operatingMargin', '-');
    } else {
  ...
  clearBtn.addEventListener('click', () => {
    ...
    document.querySelectorAll('#calc-form input').forEach(input => {
      ...
      if (input.id === 'investmentSecurities') {
        input.value = "0";
      } else if (input.id === 'inventory' || input.id === 'operatingIncome') {
        input.value = "-";
...
2. 期間（PL項目について）: 売上高、営業利益、当期純利益などの損益計算書（PL）項目について、「第〇四半期」などの期間が区切られた数字は絶対に取得しないでください。必ず「通期（1年分）」の数字（実績値、または「通期の部」に記載されている会社予想値・業績予想）を最優先で抽出してください。もし通期の数字（過去の実績も未来の予想も含む）が一切記載されていない場合は、四半期数値で代用せず、潔く null にしてください。
...
5. ★投資有価証券（IFRS対応・要注意）: 日本基準の場合は「投資有価証券」のみを抽出し、「投資その他の資産」の合計金額は絶対に抽出しないでください。IFRS適用企業の場合は、必ず「非流動資産（または固定資産）」の部にある「その他の金融資産」のみの数値を抽出してください。「流動資産」の中にある「その他の金融資産」は合算しないでください。また、IFRSにおける「その他の投資」という勘定科目は、その他の金融資産とは別物ですので絶対に investmentSecurities に含めずに 0扱いで無視してください。
6. 棚卸資産: 貸借対照表の流動資産の部にある「棚卸資産」（または商品・製品・原材料・仕掛品等の合計）を抽出してください。見つからない場合や記載がない場合は null にしてください。
7. ★営業利益（代用絶対禁止）: 損益計算書に「営業利益」という特定の項目が存在しない場合（IFRS適用企業など）は、「税引前利益」や「事業利益」などで絶対に代用せず、必ず null にしてください。
8. ★当期純利益: 単純な「当期純利益」ではなく、必ず「親会社株主に帰属する当期純利益」（IFRSの場合は「親会社の所有者に帰属する当期利益」）の数値を netIncome として抽出してください。
...
`;
    ...
    try {
      ...
      // マークダウンのコードブロックなどを除去し、波括弧の中だけを抽出する安全処理
      ...
      // 実際にどんなテキストが返ってきたかをログ出力しておく
      const rawContent = data.candidates?.[0]?.content?.parts?.[0]?.text || "No text";
      console.error("Raw AI Content:", rawContent);
      throw new Error("AIから返却された財務データが不正な形式でした（分析結果としては数字が取れている場合がありますが処理できませんでした）。");
    }
  };
  ...
  pdfUpload.addEventListener('change', async (e) => {
    ...
    try {
      ...
      const fillField = (id, value) => {
        const el = document.getElementById(id);
        if (!el) return;
        if ((value === null || value === undefined || value === "") && (id === 'inventory' || id === 'sales' || id === 'operatingIncome' || id === 'netIncome')) {
          el.value = "-";
      ...
      fillField('inventory', aiResult.inventory);
  ...
  fetchPdfBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // CORS回避のためプロキシを経由する
      // corsproxy.ioの仕様に合わせてURLを構築
      const proxyUrl = PROXY_URL_PREFIX + encodeURIComponent(url);
      const response = await fetch(proxyUrl);

      if (!response.ok) {
        throw new Error(`PDFのダウンロードに失敗しました (ステータス: ${response.status})。URLが間違っているか、アクセス制限されています。`);
      }
